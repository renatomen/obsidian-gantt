...
function getChangedFiles() {
  try {
    ...
    console.warn('⚠️  Could not detect changed files, assuming all files changed');
...
function main() {
  ...
  if (changedFiles.includes('**/*')) {
    console.log('⚠️  Could not determine specific changes, processing all BDD files');
  ...
  console.log(`   🏷️  Semantic tags changed: ${semanticTagChanges ? 'Yes' : 'No'}`);
  console.log(`   📝 Feature files changed: ${changedFeatures.length}`);
  console.log(`   🔧 BDD tooling changed: ${bddResult.hasChanges ? 'Yes' : 'No'}`);
